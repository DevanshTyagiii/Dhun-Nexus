...
const seed = async () => {
    ...
    // Create Dummy User
    try {
        await db.run('INSERT INTO users (username, email, password_hash) VALUES (?, ?, ?)', 'admin', 'admin@dhunnexus.com', 'hash');
    } catch (e) {
    ...
    for (const stem of SEED_STEMS) {
        try {
            await db.run(
                'INSERT INTO stems (title, bpm, key_signature, genre, likes_count, audio_url, cover_url, owner_id) VALUES (?, ?, ?, ?, ?, ?, ?, ?)',
                stem.title, stem.bpm, stem.key, stem.genre, stem.likes, stem.audioUrl, stem.coverUrl, 1
            );
        } catch (e) {
